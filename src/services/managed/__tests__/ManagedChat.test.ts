...
describe("ManagedCapabilityClient accepted Chat dispatch", () => {
  ...
  it("binds and dispatches the exact accepted lease without admission", async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(ticket, { path: "/api/plugin/chat/completions", method: "POST", capability: "chat_turn", idempotencyKey: "key-1", body, signal: undefined });
