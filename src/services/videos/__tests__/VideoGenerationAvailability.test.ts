...
import {
  ...
  getVideoGenerationAvailability,
} from "../VideoGenerationAvailability";
...
describe("VideoGenerationAvailability", () => {
  it("blocks only when plugin config v1 explicitly disables hosted videos", async () => {
    ...
    await expect(getVideoGenerationAvailability(createPlugin(), {
  ...
  it("allows authoritative true and treats a valid catalogue without the capability as unavailable", async () => {
    ...
    requestClient.responses.push(json({
      contract: "systemsculpt-plugin-config-v1",
      capabilities: { hosted_videos: true },
    }));
    requestClient.responses.push(json({
      contract: "systemsculpt-plugin-config-v1",
      capabilities: { hosted_images: true },
    }));
  ...
  it("fails open for older or unavailable servers instead of blocking the node", async () => {
    const requestClient = new QueueClient();
    requestClient.responses.push(json({
    ...
    await expect(canRunVideoGeneration(createPlugin(), {
      requestClient,
      baseUrl: "https://systemsculpt.test/api/plugin",
    })).resolves.toBe(true);
    await expect(canRunVideoGeneration(createPlugin(), {
  ...
  it("caches the last decision for five minutes to avoid repeated config probes", async () => {
    ...
    await expect(canRunVideoGeneration(plugin, {
      requestClient,
      baseUrl: "https://systemsculpt.test/api/plugin",
      now: () => currentTime,
    })).resolves.toBe(false);
