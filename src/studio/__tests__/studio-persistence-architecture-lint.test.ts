...
describe("Studio persistence architecture lint", () => {
  ...
  it("keeps load-time project repair core-owned", () => {
    ...
    const sessionControllerSource = readRepoFile(
      "src/views/studio/systemsculpt-studio-view/StudioProjectSessionController.ts"
    );
    expect(viewSource).toContain("new StudioProjectSessionController(");
    expect(viewSource).not.toContain("repairStudioProjectForLoad(");
    expect(sessionControllerSource).toContain("repairStudioProjectForLoad(");
    for (const source of [viewSource, sessionControllerSource]) {
      expect(source).not.toContain("sanitizeGraphGroups(currentProject)");
      expect(source).not.toContain("normalizeLegacyMediaNodeTitles(currentProject)");
      expect(source).not.toContain("cleanupStaleManagedOutputPlaceholders(currentProject)");
      expect(source).not.toContain("removeManagedTextOutputNodes({ project: currentProject })");
    }
  });
