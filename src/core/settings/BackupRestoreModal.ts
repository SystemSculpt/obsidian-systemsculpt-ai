...
import { createUiAction, createUiState } from "../ui/surface";
...
export class BackupSelectionModal extends StandardModal {
  ...
  onOpen(): void {
    ...
    this.renderBackups();
    (this.modalEl.ownerDocument.defaultView ?? window).setTimeout(() => search.focus(), 50);
  }
  ...
  private renderBackups(): void {
    ...
    if (filtered.length === 0) {
      createUiState(this.listEl, {
        kind: "empty",
        title: this.query ? "No matching backups" : "No backups found",
        detail: this.query ? "Try another name or date." : undefined,
      });
      return;
    }
...
export class BackupRestoreModal {
  ...
  private renderManualBackupForm(containerEl: HTMLElement, modal: BackupSelectionModal): void {
    ...
    createBackupButton.addClass("ss-backup-manual__submit");
  }
