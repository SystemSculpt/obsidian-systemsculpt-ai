...
describe("FirstPartyToolService", () => {
  let server: FirstPartyToolService;
  let mockApp: any;
  let mockPlugin: any;
  ...
  beforeEach(() => {
    ...
    for (const operation of [
      mockReadFiles, mockWriteFile, mockEditFile, mockMultiEditFiles,
      mockCreateDirectories, mockListDirectories, mockMoveItems, mockTrashFiles,
      mockFindFiles, mockGrepVault, mockManageWorkspace, mockManageContext,
    ]) operation.mockReset();
    ...
    server = new FirstPartyToolService(mockPlugin, mockApp);
  });

  describe("constructor", () => {
    it("creates instance with plugin and app", () => {
      expect(server).toBeInstanceOf(FirstPartyToolService);
    });
    ...
    describe("unknown tool", () => {
      it("throws error for unknown tool", async () => {
        await expect(server.executeTool("unknown_tool", {})).rejects.toThrow(
          "Unknown first-party tool: unknown_tool"
        );
      });
    });
  });

  describe("setVaultAllowedPaths", () => {
    it("updates allowed paths", () => {
      const newPaths = ["/notes", "/documents"];

      server.setVaultAllowedPaths(newPaths);
    ...
    it("recreates path-constrained operations without rebuilding workspace operations", () => {
      jest.clearAllMocks();

      server.setVaultAllowedPaths(["/custom"]);
      ...
      expect(ManagementOperations).not.toHaveBeenCalled();
    });

    it("handles empty paths array", () => {
      server.setVaultAllowedPaths([]);
    ...
    it("copies path values to prevent mutation", () => {
      const paths = ["/test"];
      server.setVaultAllowedPaths(paths);
  ...
  it("maps external root aliases before dispatching to path-constrained operations", async () => {
    server.setVaultRoot("Projects/Demo", ["workspace"]);
    await server.executeTool("read", { paths: ["workspace/Notes/A.md"] });
    expect(mockReadFiles).toHaveBeenCalledWith({ paths: ["Projects/Demo/Notes/A.md"] });
  });

  it("rejects pre-cancelled requests before local dispatch", async () => {
    const controller = new AbortController();
    controller.abort();
    await expect(server.executeTool("write", { path: "a.md" }, { signal: controller.signal }))
      .rejects.toMatchObject({ code: "TOOL_CANCELLED_BEFORE_START" });
    expect(mockWriteFile).not.toHaveBeenCalled();
  });

  it("preserves unknown-outcome semantics after dispatch cancellation", async () => {
    const controller = new AbortController();
    let finish!: (value: unknown) => void;
    mockWriteFile.mockImplementationOnce(() => new Promise((resolve) => { finish = resolve; }));
    const execution = server.executeTool("write", { path: "a.md" }, { signal: controller.signal });
    controller.abort();
