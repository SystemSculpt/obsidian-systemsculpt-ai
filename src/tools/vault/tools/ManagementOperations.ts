...
export class ManagementOperations {
  constructor(
    ...
    private readonly defaultChatView?: FirstPartyToolChatTarget,
  ) {}
  ...
  async manageContext(
    ...
    // up the active leaf here can target a different chat if the user changes
    // tabs while a turn is running.
    const currentChatView = originatingChatView;
    if (!currentChatView) {
      throw new Error("No originating chat view found. This tool can only be used from within a chat conversation.");
    }
