...
describe("DirectoryOperations", () => {
  ...
  describe("listDirectories", () => {
    ...
    it("lists root directory when path is .", async () => {
      ...
      const result = await dirOps.listDirectories({ paths: ["."] });
    ...
    it("returns full paths and stable offset pagination", async () => {
      const folder = new TFolder({
        path: "test",
        children: [
          new TFile({ path: "test/b/same.md", stat: { ctime: 1, mtime: 1, size: 1 } }),
          new TFile({ path: "test/a/same.md", stat: { ctime: 1, mtime: 1, size: 1 } }),
          new TFile({ path: "test/c.md", stat: { ctime: 1, mtime: 1, size: 1 } }),
        ],
      });
      (app.vault.getAbstractFileByPath as jest.Mock).mockReturnValue(folder);
      ...
      expect(result.results[0].files?.map((file) => file.path)).toEqual(["test/b/same.md"]);
      expect(result.results[0].totalItems).toBe(3);
      expect(result.results[0].nextOffset).toBe(2);
    });

    it("caps the aggregate response without advancing past omitted items", async () => {
      const children = Array.from({ length: 40 }, (_, index) => new TFile({
        path: `test/file-${String(index).padStart(2, "0")}.md`,
        stat: { ctime: 1000, mtime: 2000, size: 100 },
      }));
      const folder = new TFolder({ path: "test", children });
      (app.vault.getAbstractFileByPath as jest.Mock).mockReturnValue(folder);
      (app.vault.cachedRead as jest.Mock).mockResolvedValue("x".repeat(1000));
      ...
      for (const page of result.results) {
        ...
        expect(page.files?.map((file) => file.path)).toEqual(
          children.slice(0, shown).map((file) => file.path)
        );
        expect(page.nextOffset).toBe(shown < children.length ? shown : null);
      }
    });
  });
  ...
  describe("moveItems", () => {
    ...
    it("creates destination parents before a Vault API move", async () => {
      const mockFile = new TFile({ path: "old.md" });
      (app.vault.getAbstractFileByPath as jest.Mock).mockReturnValue(mockFile);
      const { ensureVaultFolder } = require("../../utils");

      const result = await dirOps.moveItems({
        items: [{ source: "old.md", destination: "archive/2026/new.md" }],
      });

      expect(result.results[0].success).toBe(true);
      expect(ensureVaultFolder).toHaveBeenCalledWith(app, "archive/2026");
      expect(app.fileManager.renameFile).toHaveBeenCalledWith(mockFile, "archive/2026/new.md");
    });
  ...
  describe("trashFiles", () => {
    ...
    it("refuses irreversible deletion of internal SystemSculpt paths", async () => {
      const result = await dirOps.trashFiles({ paths: [".systemsculpt/jobs/pending.json"] });

      expect(result.results[0]).toMatchObject({
        path: ".systemsculpt/jobs/pending.json",
        success: false,
        error: expect.stringContaining("Access denied"),
      });
      expect(app.vault.adapter.trashLocal).not.toHaveBeenCalled();
    });
