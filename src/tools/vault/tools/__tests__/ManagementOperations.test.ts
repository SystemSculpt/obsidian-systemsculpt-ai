...
describe("ManagementOperations", () => {
  ...
  describe("manageContext", () => {
    describe("add action", () => {
      ...
      it("throws error when no chat view is found", async () => {
        const unboundOperations = new ManagementOperations(app, mockPlugin);

        await expect(
          unboundOperations.manageContext({ action: "add", paths: ["test.md"] })
        ).rejects.toThrow("No originating chat view found");
      });
  ...
  describe("originating chat binding", () => {
    it("uses the explicitly supplied chat even when another chat is active", async () => {
      const mockFile = new TFile({ path: "test.md" });
      (app.vault.getAbstractFileByPath as jest.Mock).mockReturnValue(mockFile);
      const originatingContextManager = {
        ...mockContextManager,
        triggerContextChange: jest.fn().mockResolvedValue(undefined),
      };
      const originatingChatView = { contextManager: originatingContextManager };
      ...
      expect(originatingContextManager.triggerContextChange).toHaveBeenCalledTimes(1);
