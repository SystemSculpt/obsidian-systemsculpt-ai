...
import { YouTubeToolModule } from "./youtube/YouTubeToolModule";
import {
...
const VAULT_TOOL_NAMES = new Set<FirstPartyToolName>([
  "read",
  "write",
  "edit",
  "multi_edit",
...
export class FirstPartyToolService {
  ...
  private readonly youtubeTools: YouTubeToolModule;
  ...
  constructor(plugin: SystemSculptPlugin, app: App) {
    ...
    this.youtubeTools = new YouTubeToolModule(plugin, app);
  }

  async getAvailableTools(): Promise<ManagedToolDefinition[]> {
    return [
      ...this.vaultTools.getTools(),
      ...this.youtubeTools.getTools(),
    ].map((tool) => this.toManagedDefinition(tool));
  }
  ...
  async executeTool(
    ...
    const execution = VAULT_TOOL_NAMES.has(normalizedName)
      ? this.vaultTools.executeTool(normalizedName, this.mapVaultArgs(normalizedName, args))
      : this.youtubeTools.executeTool(normalizedName, args);
