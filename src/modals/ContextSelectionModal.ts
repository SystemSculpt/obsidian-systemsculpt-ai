...
export class ContextSelectionModal extends StandardModal {
  ...
  onOpen(): void {
    ...
    if (this.autoFocusSearch) window.setTimeout(() => this.searchInput?.focus(), 0);
  }
  ...
  private createFilterButton(
    ...
    const button = parent.createEl("button", {
      cls: `ss-context-filter-btn${selected ? " is-active" : ""}`,
      attr: { type: "button", "aria-pressed": String(selected) },
    });
    if (icon) {
      const iconElement = button.createSpan({ attr: { "aria-hidden": "true" } });
  ...
  private setFilter(filter: ContextFilter, externalButton?: HTMLElement): void {
    ...
    if (externalButton && !this.filterButtons.has(filter)) externalButton.addClass("is-active");
    this.applyFilters();
  }
  ...
  private updateLoadMoreButton(): void {
    ...
    this.loadMoreButton = item.createEl("button", {
