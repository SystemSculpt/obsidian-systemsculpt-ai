...
describe("BulkAutomationConfirmModal", () => {
  ...
  it("uses a semantic minimize button and live progress status", () => {
    ...
    const toggleButton = document.querySelector(".systemsculpt-bulk-widget-close") as HTMLButtonElement;
    expect(toggleButton.tagName).toBe("BUTTON");
    expect(toggleButton.getAttribute("aria-expanded")).toBe("true");

    const status = document.querySelector(".systemsculpt-bulk-widget-status") as HTMLElement;
    const count = document.querySelector(".systemsculpt-bulk-widget-count") as HTMLElement;
