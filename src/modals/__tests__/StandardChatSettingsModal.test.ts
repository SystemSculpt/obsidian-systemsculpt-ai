...
describe("StandardChatSettingsModal", () => {
  ...
  const openModal = (
    onChange = jest.fn<void, [ChatSettingsChange]>(),
    approvalModeDisabled = false,
  ) => {
  ...
  it("locks tool access during a run without disabling text-size preferences", () => {
    const { modal, onChange } = openModal(undefined, true);
    const ask = modal.modalEl.querySelector<HTMLButtonElement>('[data-value="ask"]')!;
    const fullAccess = modal.modalEl.querySelector<HTMLButtonElement>('[data-value="full-access"]')!;
    const medium = modal.modalEl.querySelector<HTMLButtonElement>('[data-value="medium"]')!;
    const toolAccessGroup = ask.closest<HTMLElement>('[role="radiogroup"]')!;

    expect(toolAccessGroup.getAttribute("aria-disabled")).toBe("true");
