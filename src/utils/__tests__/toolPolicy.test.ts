...
describe("first-party tool policy", () => {
  it("uses canonical names directly", () => {
    ...
    expect(splitToolName("youtube_transcript")).toEqual({
      actualName: "youtube_transcript",
      canonicalName: "youtube_transcript",
    });
  });

  it("normalizes canonical tool identities without aliases", () => {
    expect(splitToolName(" READ ").canonicalName).toBe("read");
    expect(splitToolName("youtube_transcript").canonicalName).toBe("youtube_transcript");
  ...
  it("recognizes only the six vault mutation tools", () => {
    ...
    for (const name of ["read", "list_items", "find", "search", "open", "context", "youtube_transcript", "unknown"]) {
