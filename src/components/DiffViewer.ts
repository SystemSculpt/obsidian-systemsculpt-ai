...
export class DiffViewer extends Component {
  ...
  private createHeader(): void {
    ...
    if (totalChanges === 0) {
      ...
      if (this.diffResult.stats.additions > 0) {
        const additionsSpan = stats.createSpan({
      ...
      if (this.diffResult.stats.deletions > 0) {
        ...
        const deletionsSpan = stats.createSpan({
      ...
      const totalSpan = stats.createSpan({
  ...
  private createChangeHunks(): void {
    ...
    if (hunks.length === 0) {
      const noChanges = content.createDiv({
