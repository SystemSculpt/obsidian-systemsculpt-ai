...
it('disables desktop execution on mobile while retaining navigation', () => {
  ...
  const buttons = root.querySelectorAll('button'); expect(buttons[0].disabled).toBe(true); buttons[0].click(); expect(run).not.toHaveBeenCalled();
  buttons[1].click(); expect(focus).toHaveBeenCalledWith('runs');
});
