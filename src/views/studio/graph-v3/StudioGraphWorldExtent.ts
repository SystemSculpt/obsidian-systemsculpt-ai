...
export function computeStudioWorldExtent(input: ComputeStudioWorldExtentInput): StudioWorldExtent {
  ...
  if (right - left > maxSize) {
    // Trim the side farther from the view; the view itself always fits.
    if (viewCenterX - left > right - viewCenterX) left = right - maxSize;
    else right = left + maxSize;
  }
  if (bottom - top > maxSize) {
    if (viewCenterY - top > bottom - viewCenterY) top = bottom - maxSize;
