...
export class StudioNodeContextMenuOverlay {
  ...
  open(options: {
    ...
    this.lifecycle?.show(() => {
      if (this.searchInputEl && !isMobileLayout(this.rootEl)) {
        try { this.searchInputEl.focus({ preventScroll: true }); }
        catch { try { this.searchInputEl.focus(); } catch { /* Host may reject focus. */ } }
      }
      this.applyLayout();
    });
    this.applyLayout();
