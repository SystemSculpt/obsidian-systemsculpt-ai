...
function renderInlineConfigSelectField(options: InlineConfigFieldOptions): void {
  const {
    node,
    field,
    fieldEl,
  ...
  if (field.selectPresentation === "button_group" && Array.isArray(field.options) && field.options.length > 0) {
    ...
    for (const option of field.options) {
      const buttonEl = createStudioAction(rowEl, {
        ...
        onSelect: () => {
          setValue(option.value, { mode: "discrete" });
  ...
  selectEl.addEventListener("change", () => {
    setValue(selectEl.value, { mode: "discrete" });
  });
}

function renderInlineConfigNumberField(options: InlineConfigFieldOptions): void {
  const { node, definition, field, fieldEl, interactionLocked, setValue } = options;
  ...
  const commit = (): void => {
    ...
    setValue(normalized, { mode: "discrete" });
  };
...
function renderInlineConfigTextField(options: InlineConfigFieldOptions): void {
  const { node, field, fieldEl, interactionLocked, setValue } = options;
  const inputEl = fieldEl.createEl("input", {
    cls: "ss-studio-node-inline-config-input",
  ...
  inputEl.addEventListener("input", () => {
    setValue(inputEl.value, { mode: "continuous" });
  });
}

function renderInlineConfigTextareaField(options: InlineConfigFieldOptions): void {
  const {
    node,
    field,
    fieldEl,
  ...
  textAreaEl.addEventListener("input", () => {
    setValue(textAreaEl.value, { mode: "continuous" });
  });
}

function renderInlineConfigBooleanField(options: InlineConfigFieldOptions): void {
  const { node, field, fieldEl, interactionLocked, setValue } = options;
  ...
  checkboxEl.addEventListener("change", () => {
    setValue(checkboxEl.checked, { mode: "discrete" });
  });
}

function renderInlineConfigJsonObjectField(options: InlineConfigFieldOptions): void {
  const { node, field, fieldEl, interactionLocked, setValue } = options;
  const textAreaEl = fieldEl.createEl("textarea", {
  ...
  const commit = (): void => {
    ...
    if (!raw) {
      setValue(isPortList ? [] : {}, { mode: "discrete" });
      return;
    }
    try {
      ...
      setValue(parsed as Record<string, StudioJsonValue>, { mode: "discrete" });
    } catch {
...
function renderInlineConfigStringListField(options: InlineConfigFieldOptions): void {
  const { node, field, fieldEl, interactionLocked, setValue } = options;
  const textAreaEl = fieldEl.createEl("textarea", {
    cls: "ss-studio-node-inline-config-textarea",
    attr: {
  ...
  textAreaEl.addEventListener("input", () => {
    ...
    setValue(nextValues, { mode: "continuous" });
  });
}

function renderInlineConfigNoteSelectorField(options: InlineConfigFieldOptions): void {
  const {
    node,
    field,
    fieldEl,
  ...
  const emitChange = (mutationOptions?: StudioGraphNodeMutationOptions): void => {
    ...
    updateSummary();
    setValue(serialized, mutationOptions);
  };
...
function renderInlineConfigPathField(options: InlineConfigFieldOptions): void {
  const {
    node,
    field,
    fieldEl,
  ...
  browseButtonEl.addEventListener("click", (event) => {
    ...
    void browseForNodeConfigPath(field, browseButtonEl, pathBrowseOptions).then((selected) => {
      ...
      setValue(selected, { mode: "discrete" });
    });
  });
}

type InlineConfigPanelOptions = {
  nodeEl: HTMLElement;
  node: StudioNodeInstance;
  definition: StudioNodeDefinition;
...
type InlineConfigFieldOptions = Pick<InlineConfigPanelOptions,
  "node" | "definition" | "interactionLocked" | "pathBrowseOptions" | "resolveDynamicSelectOptions" | "openMediaModelPicker"
> & {
  field: StudioNodeConfigFieldDefinition;
  fieldEl: HTMLElement;
  compactSingleRow: boolean;
  setValue: (value: StudioJsonValue, options?: StudioGraphNodeMutationOptions) => void;
};

const FIELD_RENDERERS: Partial<Record<StudioNodeConfigFieldDefinition["type"], (options: InlineConfigFieldOptions) => void>> = {
  select: renderInlineConfigSelectField,
  number: renderInlineConfigNumberField,
  media_path: renderInlineConfigPathField,
  file_path: renderInlineConfigPathField,
  directory_path: renderInlineConfigPathField,
  textarea: renderInlineConfigTextareaField,
  text: renderInlineConfigTextField,
  boolean: renderInlineConfigBooleanField,
  json_object: renderInlineConfigJsonObjectField,
  port_list: renderInlineConfigJsonObjectField,
  string_list: renderInlineConfigStringListField,
  note_selector: renderInlineConfigNoteSelectorField,
};

export function renderInlineConfigPanel(options: InlineConfigPanelOptions): boolean {
  const {
    nodeEl,
    node,
    definition,
  ...
  for (const field of fields) {
    ...
    if (!renderField) continue;
    renderField({
      node, definition, field, fieldEl, interactionLocked,
      pathBrowseOptions, resolveDynamicSelectOptions, openMediaModelPicker,
      compactSingleRow: compactTextareaFieldKeys?.has(field.key) === true,
      setValue: (value, mutationOptions) => commitInlineConfigValueChange({
        node, key: field.key, value, mutationOptions,
