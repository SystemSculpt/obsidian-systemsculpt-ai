...
describe("StudioGraphWorkspaceRenderer controls", () => {
  it("wires right-ribbon control callbacks", () => {
    ...
    renderStudioGraphWorkspace({
      ...
      graphInteraction: {
        ...
        registerAlignmentGuidesElement: jest.fn(),
      ...
      takeTextNodeEditorMountState: () => ({ isEditing: false, shouldAutoFocus: false }),
  ...
  it("renders the diagram tools row and arms/disarms tools", () => {
    ...
    expect(root.querySelector('[aria-label="Select tool"]')?.getAttribute("title")).toContain("(Shift+C or S)");
