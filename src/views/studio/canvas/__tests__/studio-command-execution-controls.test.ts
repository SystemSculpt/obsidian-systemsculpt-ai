...
it('shows saved settings on mobile without connecting', async () => {
  ...
  expect(f.model.disabled).toBe(true); expect(f.effort.disabled).toBe(true); expect(readCodexCatalog).not.toHaveBeenCalled();
  expect(f.root.querySelector('[data-testid="studio.command.models-retry"]')).toBeNull(); f.dispose();
});
