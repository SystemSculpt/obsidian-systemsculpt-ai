...
/**
 * AgentSelectionMenu provides an inline system prompt selection experience
...
export class AgentSelectionMenu extends Component {
  ...
  private createMenu(): void {
    ...
    titleContainer.createEl('span', { text: 'Switch Prompt', cls: 'agent-selection-title' });
  ...
  private render(): void {
    ...
    this.filteredAgents.forEach((agent, index) => {
      ...
      const iconName = agent.type === 'custom' ? 'file-text' :
  ...
  private isCurrentAgent(agent: AgentItem): boolean {
    const currentType = normalizeDesktopPromptSelectionType(this.chatView.systemPromptType);
    if (agent.type === 'custom') {
      return currentType === 'custom' &&
             this.chatView.systemPromptPath === agent.path;
    }
    return currentType === agent.type;
  }
  ...
  private async selectAgent(agent: AgentItem): Promise<void> {
    try {
      // Update chat view with selected prompt
      this.chatView.systemPromptType = agent.type;

      if (agent.type === 'custom' && agent.path) {
        this.chatView.systemPromptPath = agent.path;
        this.chatView.currentPrompt = await this.systemPromptService.getSystemPromptContent('custom', agent.path);
      } else {
        this.chatView.systemPromptPath = undefined;
        this.chatView.currentPrompt = agent.prompt || GENERAL_USE_PRESET.systemPrompt;
      }
      ...
      if (this.chatView.messages.length === 0 && typeof this.chatView.displayChatStatus === 'function') {
        this.chatView.displayChatStatus();
      }

      // Show confirmation
      new Notice(`Switched prompt to ${agent.name}`, 2000);

      // Remove the /agent command from input and close menu
      this.removeCommandAndClose();

    } catch (error) {
      new Notice(`Failed to switch prompt: ${error instanceof Error ? error.message : String(error)}`);
      console.error('Prompt switch error:', error);
    }
  }
