...
describe("ManagedAgentController", () => {
  ...
  it("keeps streamed reasoning summaries distinct, ordered, and durable", async () => {
    const harness = createHarness([[
      ...
      { kind: "reasoning_summary_delta", text: "Selected the smallest safe edit." },
      { kind: "content_delta", text: "The note is updated." },
      { kind: "finish_reason", reason: "stop" },
      {
        kind: "usage",
        promptTokens: 4,
    ...
    expect(result.kind).toBe("completed");
    expect(harness.controller.getSnapshot().usage).toEqual({
      promptTokens: 4,
      completionTokens: 9,
      reasoningTokens: 6,
      totalTokens: 13,
    ...
    expect(selectAgentMessageParts(
      ...
      expect.objectContaining({
        kind: "reasoning",
        state: "complete",
        summary: "Checked the current note. Selected the smallest safe edit.",
      }),
      expect.objectContaining({ kind: "text", state: "complete", markdown: "The note is updated." }),
    ]);

    const settled = harness.persisted[0].message;
    expect(settled.content).toBe("The note is updated.");
    expect(settled.messageParts?.map((part) => part.type)).toEqual(["reasoning", "content"]);
    expect(settled.messageParts?.map((part) => part.data)).toEqual([
      "Checked the current note. Selected the smallest safe edit.",
      "The note is updated.",
    ]);
    expect(Object.keys(settled)).not.toEqual(expect.arrayContaining(["reasoning", "reasoning_details"]));
    expect(reloadMessage(settled).messageParts?.map((part) => part.type)).toEqual(["reasoning", "content"]);
  });
