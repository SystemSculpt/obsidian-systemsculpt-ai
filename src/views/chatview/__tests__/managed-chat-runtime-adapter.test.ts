...
describe("ManagedChatRuntimeAdapter live events", () => {
  ...
  it("accepts provider token-detail fields while consuming canonical usage totals", async () => {
    const state = setup();
    state.requestClient.responses.push(response([
      'data: {"choices":[{"delta":{"content":"ok"}}]}',
      '',
      'data: {"choices":[{"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5,"prompt_tokens_details":{"cached_tokens":0},"completion_tokens_details":{"reasoning_tokens":0}}}',
      '',
      'data: [DONE]',
      '',
      '',
    ...
    await expect(collect(result.events)).resolves.toEqual([
      ...
      { kind: "usage", promptTokens: 3, completionTokens: 2, totalTokens: 5 },
      sessionCheckpoint(),
      { kind: "done" },
    ]);
  });
