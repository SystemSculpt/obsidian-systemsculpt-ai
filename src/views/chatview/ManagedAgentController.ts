...
class OrderedMessageStream {
  ...
  public appendText(delta: string): StreamedTextPart {
    if (!this.activeText) {
      this.activeText = {
        kind: "text",
  ...
  public appendTool(tool: StreamedTool): StreamedTextPart | undefined {
...
function addUsage(base: UsageValues, current: UsageValues): AgentUsage {
  ...
  for (const key of ["promptTokens", "completionTokens", "totalTokens", "costTotal"] as const) {
...
export class ManagedAgentController {
    ...
    try {
      ...
      while (true) {
        ...
        this.emit(active, {
          ...
          label: phase === "initial" ? "Thinking" : "Continuing",
        });
  ...
  private async streamPhase(
      ...
      switch (event.kind) {
        ...
        case "content_delta":
          ...
          this.emit(active, {
            type: "text.delta",
            messageId,
            partId: orderedStream.appendText(event.text).partId,
            delta: event.text,
          });
          break;
    ...
    const completedText = orderedStream.completeActiveText();
    if (completedText) {
      this.emit(active, { type: "text.completed", messageId, partId: completedText.partId });
    }
  ...
  private startOrderedTool(
    ...
    const completedText = orderedStream.appendTool(tool);
    if (completedText) {
      this.emit(active, { type: "text.completed", messageId, partId: completedText.partId });
    }
