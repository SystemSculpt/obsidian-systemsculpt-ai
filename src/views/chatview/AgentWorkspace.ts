...
function iconButton(parent: HTMLElement, label: string, icon: string): HTMLButtonElement {
  const element = parent.createEl("button", {
    cls: "clickable-icon systemsculpt-agent-icon-button",
...
export class AgentWorkspace extends Component {
  ...
  constructor(parent: HTMLElement, private readonly options: AgentWorkspaceOptions) {
    ...
    this.titleElement = header.createDiv({ cls: "systemsculpt-agent-header-title", text: "New chat" });
    ...
    this.creditsButton = options.onOpenCredits
      ? headerActions.createEl("button", {
          cls: "systemsculpt-agent-credits",
          attr: { type: "button", "aria-label": "Credits", title: "Credits" },
        })
      : null;
    if (this.creditsButton) {
      this.creditsButton.setText("—");
    ...
    this.banner = this.element.createDiv({ cls: "systemsculpt-agent-banner" });
    ...
    this.jumpButton = this.element.createEl("button", {
      cls: "systemsculpt-agent-jump",
      attr: { type: "button", "aria-label": "Jump to latest" },
    });
    const jumpIcon = this.jumpButton.createSpan();
    setIcon(jumpIcon, "arrow-down");
    this.jumpButton.createSpan({ text: "Latest" });
    ...
    this.queueElement = this.element.createDiv({
      ...
      attr: { "aria-label": "Queued follow-ups" },
    });
  ...
  public setCredits(label: string | null, low = false): void {
    ...
    if (label !== null) {
      this.creditsButton.setText(label);
      this.creditsButton.setAttribute("aria-label", `Credits: ${label}`);
      this.creditsButton.setAttribute("title", `Credits: ${label}`);
    }
  }
  ...
  public setReadOnly(readOnly: boolean): void {
    this.composer.setReadOnly(readOnly);
    if (readOnly) this.setBanner("Legacy chat — read-only", "info");
  }
  ...
  public setQueue(queue: readonly AgentQueuedFollowUp[]): void {
    ...
    for (const item of queue) {
      const row = this.queueElement.createDiv({ cls: "systemsculpt-agent-queue-item" });
