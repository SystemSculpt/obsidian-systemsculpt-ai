...
import type { ChatApprovalMode, ChatBackend } from "./storage/ChatPersistenceTypes";
...
function messageId(prefix: string): string {
  const crypto = window.activeWindow?.crypto ?? window.crypto;
...
export class AgentChatView extends ItemView {
  ...
  public chatBackend: ChatBackend = "systemsculpt";
  ...
  constructor(leaf: WorkspaceLeaf, plugin: SystemSculptPlugin) {
    ...
    this.contextManager = new FileContextManager({
      ...
      onContextChange: async () => {
        ...
        if (!this.contextLoading && this.chatId && this.chatBackend === "systemsculpt") {
  ...
  public async loadChatById(chatId: string): Promise<void> {
    ...
    this.workspace?.setReadOnly(this.chatBackend === "legacy");
    await this.workspace?.setHistory(loaded.messages as readonly ChatMessage[]);
    await this.workspace?.setAgentSnapshot(null);
    this.syncAttachments();
    this.workspace?.setBanner(this.chatBackend === "legacy" ? "Legacy chat — read-only" : null);
  ...
  public async saveChat(): Promise<void> {
    if (!this.chatId || this.chatBackend === "legacy") return;
    const snapshot = await this.transcript.saveMetadata();
    this.applyTranscriptIdentity(snapshot);
  }
  ...
  public async setApprovalMode(mode: ChatApprovalMode): Promise<void> {
    ...
    try {
      if (this.chatId && this.chatBackend !== "legacy") await this.saveChat();
    } catch (error) {
  ...
  public isLegacyReadOnlyChat(): boolean { return this.chatBackend === "legacy"; }
  ...
  public getAutomationSnapshot(): Record<string, unknown> {
    ...
    return {
      ...
      isLegacyReadOnly: this.isLegacyReadOnlyChat(),
    };
  }
  ...
  private async prepareSubmission(submission: AgentComposerSubmit): Promise<AgentComposerSubmit> {
    ...
    const attachments = await this.attachmentStore.externalizeAttachments(submission.attachments);
  ...
  private async executeSubmission(
    ...
    if (this.chatBackend === "legacy") throw new Error("Legacy chats are read-only.");
  ...
  private async prepareRetry(messageIdToRetry: string): Promise<void> {
    ...
    const draft = restoreChatMessageDraft(message as ChatMessage);
    this.pendingRetry = {
      kind: "resend",
  ...
  private async startNewChat(
    ...
    this.applyTranscriptIdentity(snapshot);
    this.workspace?.setTitle(this.chatTitle);
    this.workspace?.setReadOnly(false);
    this.workspace?.setBanner(null);
  ...
  private applyTranscriptIdentity(snapshot: AgentTranscriptSnapshot): void {
    ...
    this.chatBackend = snapshot.backend;
    this.workspace?.setTitle(this.chatTitle);
  }
  ...
  private installWorkspaceBindings(): void {
    const sync = (): void => this.syncAttachments();
    document.addEventListener("systemsculpt:context-changed", sync);
    document.addEventListener("systemsculpt:context-processing-changed", sync);
    this.register(() => {
      document.removeEventListener("systemsculpt:context-changed", sync);
      document.removeEventListener("systemsculpt:context-processing-changed", sync);
    });
