import { createPluginArtifactIdentityPlugin } from "./plugin-artifact-identity.mjs";
...
export function createPluginBuildOptions({
  ...
  if (overrides && Object.hasOwn(overrides, 'plugins')) {
    throw new Error('Plugin build overrides cannot replace internal plugins');
  }
  for (const key of [
    '__SS_BUILD_STAMP__',
  ...
  delete safeOverrides.plugins;

  const artifactIdentityPlugin = createPluginArtifactIdentityPlugin({
    enabled: testDriver === true,
  });
