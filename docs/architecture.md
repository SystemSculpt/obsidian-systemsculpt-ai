...
| Studio persistence | `src/studio/StudioProjectStore.ts`, `document/`, `persistence/` | One readable `.systemsculpt` document per project: validated import, three-way merged UI, agent and external edits, deletion tombstones, atomic publication, and independent support-file reconciliation. |
