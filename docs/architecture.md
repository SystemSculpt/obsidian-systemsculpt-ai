...
| Studio persistence | `src/studio/StudioProjectStore.ts`, `document/`, `persistence/` | One readable `.systemsculpt` document per project: validated import, three-way merged UI and agent edits, stamped field merges of copies from other devices with per-device clocks and deletion tombstones, atomic publication, and independent support-file reconciliation. |
